...
const PV_TECH_SHORT = {
  mono_perc: 'Mono PERC', poly: 'Poly', cdte: 'CdTe TF', cigs: 'CIGS TF',
  topcon: 'TOPCon', hjt: 'HJT', bifacial_perc: 'Bifacial PERC',
  topcon_bi: 'Bifacial TOPCon', hjt_bi: 'Bifacial HJT',
  a_si: 'a-Si', organic: 'OPV',
};
...
function Box({ x, y, w, h, fill = '#fff', stroke = '#374151', rx = 8, children }) {
  return (
    <g>
...
function Label({ x, y, text, bold = false, size = 11, fill = '#111827', anchor = 'middle' }) {
  return (
    <text x={x} y={y} textAnchor={anchor} fontSize={size} fontWeight={bold ? '600' : '400'} fill={fill} fontFamily="system-ui, sans-serif">
      {text}
    </text>
  );
}

function SubLabel({ x, y, text, size = 9.5, fill = '#6B7280', anchor = 'middle' }) {
  return (
...
function HArrow({ x1, y, x2, color = '#374151', label = '' }) {
  const mid = (x1 + x2) / 2;
  return (
    <g>
      <line x1={x1} y1={y} x2={x2 - 6} y2={y} stroke={color} strokeWidth={2} />
      <polygon points={`${x2},${y} ${x2 - 8},${y - 5} ${x2 - 8},${y + 5}`} fill={color} />
      {label && <text x={mid} y={y - 5} textAnchor="middle" fontSize={9} fill={color} fontFamily="system-ui, sans-serif">{label}</text>}
    </g>
  );
}

/** Vertical arrow from (x,y1) to (x,y2) */
function VArrow({ x, y1, y2, color = '#374151', label = '' }) {
  const dir = y2 > y1 ? 1 : -1;
  const tip = y2;
  const mid = (y1 + y2) / 2;
  return (
    <g>
      ...
      {label && <text x={x + 6} y={mid} textAnchor="start" fontSize={9} fill={color} fontFamily="system-ui, sans-serif">{label}</text>}
    </g>
  );
}

/** L-shaped connector: horizontal then vertical */
function LArrow({ x1, y1, x2, y2, color = '#374151' }) {
  const dir = y2 > y1 ? 1 : -1;
  return (
    <g>
...
function PVArrayBlock({ cx, cy, kwp, panelTech, numStrings }) {
  const w = 120, h = 80;
  ...
  return (
    <Box x={x} y={y} w={w} h={h} fill="#FEFCE8" stroke="#D97706">
      {/* Mini panel icon */}
      {[0, 1, 2].map(i => (
        <rect key={i} x={x + 12 + i * 15} y={y + 10} width={11} height={16} rx={1}
          fill="#FCD34D" stroke="#D97706" strokeWidth={1} />
      ))}
      <Label x={cx} y={y + 36} text="PV Array" bold size={11} fill="#92400E" />
      <SubLabel x={cx} y={y + 50} text={`${fmt(kwp, 1)} kWp`} fill="#B45309" />
      <SubLabel x={cx} y={y + 62} text={`${PV_TECH_SHORT[panelTech] || panelTech} · ${numStrings} string${numStrings > 1 ? 's' : ''}`} fill="#78716C" />
    </Box>
  );
}

function DCCombinerBlock({ cx, cy }) {
  const w = 90, h = 68;
  const x = cx - w / 2, y = cy - h / 2;
  return (
...
function InverterBlock({ cx, cy, topology, pvKwp, bessKw }) {
  const w = 120, h = 80;
  const x = cx - w / 2, y = cy - h / 2;
  ...
  return (
    <Box x={x} y={y} w={w} h={h} fill="#EFF6FF" stroke="#2563EB">
      {/* Squiggly AC symbol */}
      <text x={cx} y={y + 20} textAnchor="middle" fontSize={13} fill="#1D4ED8" fontFamily="system-ui">⚡</text>
...
function BatteryBlock({ cx, cy, kwh, kw, chemistry }) {
  const w = 120, h = 80;
  const x = cx - w / 2, y = cy - h / 2;
  return (
    <Box x={x} y={y} w={w} h={h} fill="#F0FDF4" stroke="#16A34A">
      <text x={cx} y={y + 20} textAnchor="middle" fontSize={13} fill="#15803D" fontFamily="system-ui">🔋</text>
...
function LoadBlock({ cx, cy, annualKwh }) {
  const w = 100, h = 68;
  const x = cx - w / 2, y = cy - h / 2;
  return (
    <Box x={x} y={y} w={w} h={h} fill="#F8FAFC" stroke="#475569">
      <text x={cx} y={y + 20} textAnchor="middle" fontSize={13} fill="#334155" fontFamily="system-ui">🏭</text>
      <Label x={cx} y={y + 36} text="Site Loads" bold size={10} fill="#0F172A" />
      {annualKwh && <SubLabel x={cx} y={y + 52} text={`~${Number(annualKwh).toLocaleString()} kWh/yr`} fill="#64748B" />}
    </Box>
  );
}

function GridBlock({ cx, cy, topology }) {
  const w = 100, h = 68;
  const x = cx - w / 2, y = cy - h / 2;
  const isHybrid = topology === 'hybrid';
  return (
    <Box x={x} y={y} w={w} h={h} fill="#FEF2F2" stroke="#DC2626">
      <text x={cx} y={y + 20} textAnchor="middle" fontSize={13} fill="#DC2626" fontFamily="system-ui">⚡</text>
...
function SectionTitle({ x, y, text }) {
  return (
    <text x={x} y={y} fontSize={10} fill="#9CA3AF" fontFamily="system-ui, sans-serif" fontWeight="500" letterSpacing="0.05em">
      {text.toUpperCase()}
    </text>
  );
}
 ...
 * PV → Combiner → Inverter → AC Panel → Load
 *                                ↕ Grid
 */
function GridTiedSchematic({ design, annualKwh }) {
  const pvKwp = design?.pv_capacity_kwp;
  const numStrings = Math.max(1, Math.ceil((pvKwp * 1000) / 4000));
  ...
  const xPV = 75, xComb = 200, xInv = 360, xAC = 520, xLoad = 700;
  const xGrid = 520;
 ...
 *
 * PV → Combiner → Hybrid Inverter → AC Panel → Load
 *                       ↕                 ↕
 *                  Battery Bank         Grid
 */
function GridTiedBessSchematic({ design, annualKwh }) {
  const pvKwp = design?.pv_capacity_kwp;
  const bessKwh = design?.bess_capacity_kwh;
  const bessKw = design?.bess_power_kw;
  const chemistry = design?.bess_chemistry?.toUpperCase() || 'LFP';
  const numStrings = Math.max(1, Math.ceil((pvKwp * 1000) / 4000));
  const panelTech = design?.pv_technology;
  const topology = design?.grid_topology;

  const W = 860, H = 400;
  const midY = 160;
  const bottomY = 310;

  const xPV = 75, xComb = 210, xInv = 375, xAC = 545, xLoad = 720;
  const xBatt = 375, xGrid = 545;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: W, display: 'block' }}>
      ...
      {/* Vertical — inverter ↔ battery, ac panel ↔ grid */}
 ...
 * Off-grid:
 *
 * PV → Combiner → MPPT CC → Battery Bank → Off-Grid Inverter → AC Panel → Load
 */
function OffGridSchematic({ design, annualKwh }) {
  const pvKwp = design?.pv_capacity_kwp;
  const bessKwh = design?.bess_capacity_kwh;
  const bessKw = design?.bess_power_kw;
  const chemistry = design?.bess_chemistry?.toUpperCase() || 'LFP';
  ...
  const xPV = 70, xComb = 200, xMPPT = 330, xBatt = 480, xInv = 640, xLoad = 800;
...
export default function SolarSchematic({ design, result }) {
  const topology = design?.grid_topology || 'grid_tied';
  const annualKwh = result?.annual_load_kwh;
  const hasBess = (design?.bess_capacity_kwh || 0) > 0;

  // Derive string config label
  const pvKwp = design?.pv_capacity_kwp || 0;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-4 bg-forest-900 text-white rounded-xl">
        ...
        <div className="flex flex-wrap gap-2 text-xs text-green-200">
          ...
          {hasBess && <span className="bg-white/10 px-2 py-1 rounded">{fmt(design.bess_capacity_kwh, 1)} kWh BESS</span>}
          <span className="bg-white/10 px-2 py-1 rounded">~{numStrings} string{numStrings > 1 ? 's' : ''}</span>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            <tr className="bg-white dark:bg-gray-900">
              ...
              <td className="px-4 py-2.5 text-gray-700 dark:text-gray-300">
                {fmt(pvKwp, 1)} kWp · {PV_TECH_SHORT[design?.pv_technology] || design?.pv_technology || '—'}
              </td>
              <td className="px-4 py-2.5 text-gray-500 text-xs">
                ~{estimatedPanels} × 400 Wp panels est. · {numStrings} string{numStrings > 1 ? 's' : ''} · Tilt {design?.pv_tilt_deg || '—'}° · Az {design?.pv_azimuth_deg || '—'}°
              </td>
            </tr>
            <tr className="bg-gray-50/50 dark:bg-gray-800/50">
              ...
              <td className="px-4 py-2.5 text-gray-500 text-xs">One fuse per string. 1000 V DC rated (NEC/IEC 62548)</td>
            </tr>
            {topology !== 'off_grid' && (
              <tr className="bg-white dark:bg-gray-900">
                <td className="px-4 py-2.5 font-medium text-blue-700">
                  {hasBess ? 'Hybrid Inverter' : 'String Inverter'}
                </td>
                <td className="px-4 py-2.5 text-gray-700 dark:text-gray-300">
            ...
            {hasBess && (
              <tr className="bg-white dark:bg-gray-900">
                ...
                <td className="px-4 py-2.5 text-gray-700 dark:text-gray-300">
                  {fmt(design.bess_capacity_kwh, 1)} kWh · {fmt(design.bess_power_kw, 1)} kW · {design?.bess_chemistry?.toUpperCase() || 'LFP'}
                </td>
                <td className="px-4 py-2.5 text-gray-500 text-xs">
                  Min SOC {design?.bess_min_soc != null ? (design.bess_min_soc * 100).toFixed(0) : '—'}% · RTE {design?.bess_round_trip_efficiency != null ? (design.bess_round_trip_efficiency * 100).toFixed(0) : '—'}% · Strategy: {(design?.bess_dispatch_strategy || '').replace(/_/g, ' ')}
                </td>
              </tr>
            )}
            <tr className="bg-gray-50/50 dark:bg-gray-800/50">
              ...
              <td className="px-4 py-2.5 text-gray-500 text-xs">Per load circuit. Earth leakage protection. IEC 60364 compliant</td>
            </tr>
            {topology !== 'off_grid' && (
              <tr className="bg-white dark:bg-gray-900">
                ...
                <td className="px-4 py-2.5 text-gray-500 text-xs">
                  {topology === 'hybrid' ? 'Automatic Transfer Switch for islanding. Synch relay for reconnect.' : 'NERC net-metering compliant where available'}
                </td>
              </tr>
            )}
          </tbody>
      ...
      <p className="text-xs text-gray-400 dark:text-gray-500 leading-relaxed px-1">
        ⚠ This schematic is a design-intent diagram generated from simulation parameters. Panel count (est. ~{estimatedPanels}) assumes 400 Wp average.
        Actual string configuration, cable sizing, and protection device ratings must be verified by a qualified electrical engineer before installation.
