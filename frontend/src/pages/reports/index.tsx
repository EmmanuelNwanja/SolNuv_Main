...
import { reportsAPI, nercAPI, downloadBlob } from '../../services/api';
...
export default function Reports() {
  ...
  const [nercCycles, setNercCycles] = useState([]);
  const [loadingNerc, setLoadingNerc] = useState(true);
  ...
  useEffect(() => {
    setLoadingNerc(true);
    nercAPI.listMyReportingCycles({ limit: 8 })
      .then((res) => setNercCycles(res.data.data || []))
      .catch(() => setNercCycles([]))
      .finally(() => setLoadingNerc(false));
  }, []);
  ...
  return (
    <>
      ...
      <div className="max-w-3xl space-y-6">
        ...
        <MotionSection className="card">
          <h2 className="font-semibold text-forest-900 flex items-center gap-2 mb-4">
            <RiHistoryLine /> NERC Reporting Cycles (Mini-Grid 2026)
          </h2>
          {loadingNerc ? (
            <div className="flex justify-center py-8"><LoadingSpinner /></div>
          ) : nercCycles.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-6">No NERC reporting cycles assigned yet.</p>
          ) : (
            <div className="space-y-2">
              {nercCycles.map((cycle) => (
                <div key={cycle.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50">
                  <div>
                    <p className="text-sm font-medium text-slate-700">{cycle.project_name}</p>
                    <p className="text-xs text-slate-400 mt-0.5">
                      {cycle.cadence} · {new Date(cycle.period_start).toLocaleDateString('en-NG')} - {new Date(cycle.period_end).toLocaleDateString('en-NG')}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-slate-500">Due {new Date(cycle.due_date).toLocaleDateString('en-NG')}</p>
                    <span className="text-xs rounded-full bg-white px-2 py-1 border border-slate-200">{cycle.status}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 text-xs text-slate-500">
            Open each project&apos;s regulatory workspace to submit NERC applications and filing evidence.
          </div>
          <div className="mt-2">
            <Link href="/projects" className="text-xs font-semibold text-forest-900 hover:underline">Go to Projects →</Link>
          </div>
        </MotionSection>
      </div>
    </>
