...
type StatTone = 'emerald' | 'amber' | 'blue' | 'violet' | 'rose' | 'slate';
...
function AnalyticsPage() {
  ...
  const revenueByPlanRows = Object.entries(d?.finance?.revenue_by_plan || {})
    .map(([plan, amt]) => ({ plan, amount_ngn: amt }))
    .sort((a, b) => Number(b.amount_ngn) - Number(a.amount_ngn));
  const calcTypeRows = Object.entries(d?.users?.calculator_usage_by_type || {})
    .map(([type, count]) => ({ type, count }))
    .sort((a, b) => Number(b.count) - Number(a.count));
  const blogReadsByPost = Object.entries(d?.blog?.reads_by_post || {})
    .map(([post_id, reads]) => ({ post_id, reads }))
    .sort((a, b) => Number(b.reads) - Number(a.reads))
    .slice(0, 10);
  const blogClicksByPost = Object.entries(d?.blog?.clicks_by_post || {})
    .map(([post_id, clicks]) => ({ post_id, clicks }))
              ...
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    ...
                    <SimpleTable
                      columns={[
                        { key: 'post_id', label: 'Post ID', render: (r) => <span className="font-mono text-xs text-slate-400">{String(r.post_id).slice(0, 8)}…</span> },
                        { key: 'reads', label: 'Reads', render: (r) => <span className="font-semibold text-emerald-400">{Number(r.reads).toLocaleString()}</span> },
                      ]}
                  ...
                  <div>
                    ...
                    <SimpleTable
                      columns={[
                        { key: 'post_id', label: 'Post ID', render: (r) => <span className="font-mono text-xs text-slate-400">{String(r.post_id).slice(0, 8)}…</span> },
                        { key: 'clicks', label: 'Clicks', render: (r) => <span className="font-semibold text-violet-400">{Number(r.clicks).toLocaleString()}</span> },
                      ]}
              ...
              <>
                ...
                <SimpleTable
                  columns={[
                    ...
                    { key: 'views', label: 'Views', render: (r) => <span className="font-semibold text-blue-400">{Number(r.views).toLocaleString()}</span> },
                    { key: 'pct', label: '% of Total', render: (r) => {
                      const total = d?.pages?.total_views || 0;
                      const views = Number(r.views);
                      return <span className="text-slate-400">{total ? ((views / Number(total)) * 100).toFixed(1) + '%' : '—'}</span>;
                    }},
                  ]}
                  rows={topPagesRows}
              ...
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    ...
                    <SimpleTable
                      columns={[
                        { key: 'plan', label: 'Plan', render: (r) => <span className="capitalize font-medium text-slate-200">{String(r.plan)}</span> },
                  ...
                  <div>
                    ...
                    <div className="mt-4">
                      ...
                      <SimpleTable
                        columns={[
                          { key: 'company_id', label: 'Company ID', render: (r) => <span className="font-mono text-xs text-slate-400">{String(r.company_id).slice(0, 8)}…</span> },
                          { key: 'amount_ngn', label: 'Revenue', render: (r) => <span className="font-semibold text-amber-400">{formatMoney(r.amount_ngn)}</span> },
                        ]}
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  ...
                  <StatCard
                    ...
                    value={Object.values(d?.users?.calculator_usage_by_type || {})
                      .reduce((s, c) => Number(s) + Number(c), 0)
                      .toLocaleString()}
                    sub="total tool invocations"
                    color="amber"
                  />
                </div>
                ...
                <SimpleTable
                  columns={[
                    { key: 'type', label: 'Calculator Type', render: (r) => <span className="capitalize font-medium text-slate-200">{String(r.type).replace(/-/g, ' ')}</span> },
                    { key: 'count', label: 'Uses', render: (r) => <span className="font-semibold text-amber-400">{Number(r.count).toLocaleString()}</span> },
                  ]}
                  rows={calcTypeRows}
