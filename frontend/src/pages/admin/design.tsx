...
type KpiTone = 'emerald' | 'amber' | 'blue' | 'violet' | 'rose';
    ...
    <div className="space-y-6">
      ...
      {Object.keys(d.topology_distribution || {}).length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {Object.entries(d.topology_distribution).sort((a, b) => Number(b[1]) - Number(a[1])).map(([topo, count]) => (
      ...
      {Object.keys(d.installation_type_distribution || {}).length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {Object.entries(d.installation_type_distribution).sort((a, b) => Number(b[1]) - Number(a[1])).map(([type, count]) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data.designs_by_plan || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([plan, count]) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data.simulations_by_plan || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([plan, count]) => (
            <div key={plan} className={`rounded-xl px-4 py-3 ${planColors[plan] || 'bg-slate-50 text-slate-600'}`}>
              <p className="text-[11px] font-bold uppercase tracking-wide opacity-70">{plan}</p>
              <p className="text-xl font-bold mt-0.5">{fmt(count)}</p>
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data.dispatch_strategy_breakdown || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([strat, count]) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data.load_profile_sources || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([src, count]) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data.topology_breakdown || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([topo, count]) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data.installation_type_breakdown || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([type, count]) => (
            <div key={type} className="rounded-xl bg-violet-50 text-violet-700 px-4 py-3">
              <p className="text-[11px] font-bold uppercase tracking-wide opacity-70">{type.replace(/_/g, ' ')}</p>
              <p className="text-xl font-bold mt-0.5">{fmt(count)}</p>
            </div>
