        ...
        <div className="mb-5 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-amber-800">
              {usageData.plan === 'free' ? 'Free Tier' : 'Basic Plan'} Usage — {usageData.total_used ?? Object.values(usageData.usage || {}).reduce((a, b) => Number(a) + Number(b), 0)} / {usageData.total_limit ?? (usageData.plan === 'free' ? 6 : 54)} calculations used this month
            </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" className="input" value={panelForm.size_watts} min="50" max="800"
                    onChange={e => setPanelForm(f => ({ ...f, size_watts: Number(e.target.value) }))} />
                </div>
                <div>
                  <label className="label">Number of Panels</label>
                  <input type="number" className="input" value={panelForm.quantity} min="1"
                    onChange={e => setPanelForm(f => ({ ...f, quantity: Number(e.target.value) }))} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" className="input" value={batteryForm.capacity_kwh} min="0.5" step="0.1"
                    onChange={e => setBatteryForm(f => ({ ...f, capacity_kwh: Number(e.target.value) }))} />
                </div>
                <div>
                  <label className="label">Number of Units</label>
                  <input type="number" className="input" value={batteryForm.quantity} min="1"
                    onChange={e => setBatteryForm(f => ({ ...f, quantity: Number(e.target.value) }))} />
                </div>
              </div>
                  ...
                  <div className="card space-y-3">
                    ...
                    {degradResult.climate_stressors && (
                      <div className="pt-2 border-t border-slate-100 space-y-2">
                        ...
                        {Object.entries(degradResult.climate_stressors).map(([key, raw]) => {
                          const s = raw as { severity: string; factor: number; description: string };
                          return (
