  ...
  async function loadMarketPrices() {
    try {
      ...
      if (res.data.data) {
        setForm((f) => ({
          ...f,
          panel_cost_per_unit: String(
            (Number(res.data.data.panels?.per_watt_ngn) || 0) * Number(f.panel_watts)
          ),
          battery_cost_per_kwh: String(res.data.data.batteries?.lfp_per_kwh ?? ''),
          inverter_cost_per_kw: String(res.data.data.inverters?.hybrid_per_kw ?? ''),
        }));
      }
    } catch (err) {
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <input type="number" className="input" placeholder="Qty" value={newCustomItem.quantity} onChange={e => setNewCustomItem(p => ({ ...p, quantity: Number(e.target.value) }))} />
