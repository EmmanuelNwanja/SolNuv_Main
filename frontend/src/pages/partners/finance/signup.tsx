...
import { hasPartnerFinancier } from "../../../utils/partnerPortal";
import toast from "react-hot-toast";

export default function PartnerFinanceSignupPage() {
  ...
  useEffect(() => {
    ...
    if (session && hasPartnerFinancier(profile)) {
      void router.replace("/partners/finance");
    }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!session) {
      ...
      void router.push(`/login?next=${encodeURIComponent("/partners/finance/signup")}`);
      return;
    }
    ...
    try {
      ...
      await v2API.registerActor({
        ...
        actor_type: "financier",
        role_title: "member",
        jurisdiction: "NG",
      });
      await refreshProfile();
      toast.success("Financier organization linked to your account");
      void router.replace("/partners/finance");
    } catch (err: unknown) {
  ...
  return (
    <>
      <Head>
        <title>Finance partner signup — SolNuv</title>
      </Head>
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-6">
          <div>
            <h1 className="text-xl font-bold text-forest-900">Finance partner</h1>
            <p className="text-sm text-slate-500 mt-1">
              Register your financier organization to view escrow summaries and submit funding interest.
            </p>
          </div>
          {!session ? (
            <div className="space-y-3">
              ...
              <Link
                href={`/login?next=${encodeURIComponent("/partners/finance/signup")}`}
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Solar Capital Partners"
                  required
                />
              </div>
              ...
              <button type="submit" disabled={busy} className="btn-primary w-full">
                {busy ? "Saving…" : "Register financier organization"}
              </button>
            </form>
          )}
        </div>
